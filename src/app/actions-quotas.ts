...
export async function updateCategoryQuotaAction(categoryName: string, availableSpots: number) {
    const supabase = await createClient();
    try {
        ...
        const { error } = await supabase
            .from('category_quotas')
            .update({ available_spots: availableSpots, updated_at: new Date().toISOString() })
