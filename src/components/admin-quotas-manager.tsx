...
export default function AdminQuotasManager() {
    ...
    const updateQuota = async (category: string, currentVal: number, change: number) => {
        const newVal = Math.max(0, currentVal + change);
        if (newVal === currentVal) return;

        setUpdating(category);
        const res = await updateCategoryQuotaAction(category, newVal);
        if (res.success) {
            setQuotas(prev => prev.map(q => q.category_name === category ? { ...q, available_spots: newVal } : q));
        } else {
                    ...
                    {quotas.map(quota => (
                        <div key={quota.category_name} className={`flex items-center justify-between p-4 rounded-xl border ${quota.available_spots > 0 ? 'bg-emerald-500/5 border-emerald-500/30 ring-1 ring-emerald-500/20' : 'bg-white/5 border-white/10'} transition-all`}>
                            ...
                            <div className="flex items-center gap-3">
                                <button
                                    onClick={() => updateQuota(quota.category_name, quota.available_spots, -1)}
                                ...
                                <div className={`w-8 text-center font-black ${quota.available_spots > 0 ? 'text-emerald-400 text-xl' : 'text-gray-500 text-lg'} ${updating === quota.category_name ? 'animate-pulse opacity-50' : ''}`}>
                                    {quota.available_spots}
                                </div>
